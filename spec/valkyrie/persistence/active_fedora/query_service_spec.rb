  ...
  describe "find_members" do
    context "when they come from solr" do
      it "can find properties and member_ids of sub-resources" do
        nested_child = persister.save(resource: Book.new)
        middle_child = persister.save(resource: Book.new(member_ids: nested_child.id, title: "Test", read_groups: ["public"], read_users: ["tpend"]))
        parent = persister.save(resource: Book.new(member_ids: middle_child.id))
