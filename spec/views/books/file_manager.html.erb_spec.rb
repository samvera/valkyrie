...
RSpec.describe "books/file_manager.html.erb", type: :view do
  let(:members) { [member] }
  let(:member) { BookChangeSet.new(Persister.save(model: Book.new)) }
  let(:parent) { BookChangeSet.new(Persister.save(model: Book.new(title: "Test title", member_ids: members.map(&:id)))) }
